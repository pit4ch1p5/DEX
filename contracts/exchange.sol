...
contract TokenExchange {
        ...
        payable
    {
        ...
        */
        ...
        // require caller has enough tokens to match the ETH they're sending
        uint amountTokens = msg.value.div(priceToken());
        require (amountTokens < token.balanceOf(msg.sender), "Insufficient tokens to match your ETH deposit.");
        ...
        totalLiquidity.add(msg.value.div(eth_reserves).mul(totalLiquidity));
        ...
        payable 
    {
        ...
        */
        ...
        uint fees;
        uint tokensWithdrawn = token_reserves.mul(fees).mul(msg.value).div(eth_reserves.add(fees.mul(msg.value))) ;
        
        // require enough ETH and sender owns the amount requested 
        require (tokensWithdrawn < token_reserves, "Insufficient tokens in pool");
        require (token.transferFrom(address(this), msg.sender, tokensWithdrawn), "Not enough tokens in smart contract");
        
        // reset reserves 
        eth_reserves.add(msg.value);
        token_reserves.sub(tokensWithdrawn);
        k = eth_reserves.mul(token_reserves);
